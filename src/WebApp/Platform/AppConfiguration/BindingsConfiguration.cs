...
public static class BindingsConfiguration
{
    public static void BindSettings(WebApplicationBuilder builder)
    {
        ...
        AppSettings.OrgNotificationsApiUrl =
            builder.Configuration.GetValue<string>(nameof(AppSettings.OrgNotificationsApiUrl)) ?? string.Empty;
        ...
        if (segments.Length > 1)
            AppSettings.SupportSettings.InformationalBuild = segments[1].Substring(0, Math.Min(7, segments[1].Length));
